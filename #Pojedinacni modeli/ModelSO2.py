...
dftrain = pd.read_csv('cesticecsv/SO2train.csv')
dfeval = pd.read_csv('cesticecsv/SO2Test.csv')
...
model = keras.Sequential([
    ...
    keras.layers.Dense(26, activation='softmax') # output layer (3)
])
...
k=0
for i in range(132):    
    print(np.argmax(predictions[i]),' razmal'  ,predictions[i] ,end=', ')
    print(y_test[i])
